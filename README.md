...
def position(x1, y1, x2, y2, theta_1, theta_2, t):
    ...
    plt.ylabel(r"$y/L$-->")
    plt.xlabel(r"$x/L$-->")
    plt.figtext(0.05,0.07, "$m_1$=50kg,$m_2$=1kg")
...
def phasespace(theta1, w1, theta2, w2):
    ...
    plt.figtext(0.08,0.06, "$m_1$=50kg , $m_2$=1kg")
...
def theta1v2(theta1, theta2 ):
    ...
    plt.xlabel(r"$\theta_1$-->")
    plt.ylabel(r"$\theta_2$-->")
    plt.figtext(0.05,0.04, "$m_1$=50kg,$m_2$=1kg")
...
theta_01 = ((np.pi)/180)*180
theta_02 = ((np.pi)/180)*179
